...
def send_post(url, sign, param):
    ...
    try:
        ...
        result = {'code': '666666', 'Error': '%s' % e, 'data': '%s' % str(data)}
...
def send_mesage(old_version, new_version):
    data1 = {
        ...
        "content": "检测到版本更新：新版本号：%s，更新时间:%s" % (old_version, str(datetime.datetime.now()).split('.')[0])
    }
    b64_data1 = base64.b64encode(str(data1).encode('utf-8')).decode('utf-8')
    print(data1)
    return b64_data1
...
# 主函数
def mes_main(old_version, new_version):
    try:
        # 获取登陆业务参数
        dl_m = get_dlmes()
        # 获取登陆授权sign
        ...
        res = {'code': '444444', 'Error': e}
    return res


if __name__ == '__main__':
    s = mes_main('4.0.02', '4.0.03')
    print(s)
